...
class Master(object):
    ...
    def execute(
        ...
        # read file
        elif function_code == defines.READ_FILE:
            is_file_function = True
            len_bytes = 0x07
            ref_type = 0x06
            file_number = 0x01
            pdu = struct.pack(">BBBHHH", function_code, len_bytes, ref_type, file_number, starting_address, quantity_of_x)
            if not data_format:
                data_format = ">" + (quantity_of_x * "H")
            if expected_length < 0:
                # No length was specified and calculated length can be used:
                # slave + func + bytcodeLen + fileresplen + reftype + bytecode x 2 + crc1 + crc2
                expected_length = 2 * quantity_of_x + 7
        else:
...
class Slave(object):
    ...
    def __init__(self, slave_id, unsigned=True, memory=None):
        ...
        self._fn_code_map = {
            ...
            defines.READ_FILE: self._read_file,
        }
    ...
    def _read_file(self, request_pdu):
        """handle read file function"""
        call_hooks("modbus.Slave.handle_read_file_request", (self, request_pdu))
        return self._read_registers(defines.FILE, request_pdu)
